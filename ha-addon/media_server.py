...
<head>
    ...
    <title>Media Studio • Audio Stream Hub</title>
    ...
    <script>
        ...
        async function fetchStatus() {
            try {
                ...
                // Check for completion notification (strictly guarded by appSettings.notifications_enabled)
                const currentJobStatus = data.active_job ? data.active_job.status : '';
                if (lastJobStatus === 'downloading' && !data.active_job) {
                    if (appSettings.notifications_enabled && "Notification" in window && Notification.permission === "granted") {
                        const notif = new Notification(currentLang === 'es' ? "🎵 Media Studio: Descarga Finalizada" : "🎵 Media Studio: Download Complete", {
                            body: currentLang === 'es' ? "Las canciones se descargaron en tu biblioteca." : "Downloaded tracks are now organized in your music library.",
                            icon: "/favicon.svg"
                        });
                        notif.onclick = () => { window.focus(); notif.close(); };
                    }
        ...
        async function submitDownload() {
            ...
            if (appSettings.notifications_enabled && "Notification" in window && Notification.permission === "default") {
